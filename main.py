...
def _fuzzy_find(columns, patterns, default=None):
    """Return first column whose name matches any regex pattern (case-insensitive)."""
...
def detect_schema(df, source_hint=""):
    """Auto-detect semantic columns in any log DataFrame."""
...
with st.sidebar:
    ...
    st.divider()
    st.markdown(f"<p style='color:{TEXT_SEC};font-size:0.7rem;text-transform:uppercase;letter-spacing:1px'>Data Source</p>", unsafe_allow_html=True)
    uploaded_files = st.file_uploader("Upload Logs (CSV)", accept_multiple_files=True, type="csv")
...
dfs, schemas = load_data(uploaded_files)
...
# ─────────────────────────────────────────────────────────────────────────────
# External IPs hitting firewall
ext_fw_ips = set()
if fw_sc.get("src_ip") and fw_sc["src_ip"] in fw.columns:
    ext_fw_ips = set(fw[~fw[fw_sc["src_ip"]].apply(is_internal)][fw_sc["src_ip"]].dropna().unique())

# Failed logins
failed_logins = 0
brute_force_ips = set()
if auth_sc.get("action") and auth_sc["action"] in auth.columns:
    fail_mask = auth[auth_sc["action"]].str.contains("fail|fail.*login|denied", case=False, na=False, regex=True)
    failed_logins = fail_mask.sum()
    if auth_sc.get("src_ip") and auth_sc["src_ip"] in auth.columns:
        ip_fails = auth[fail_mask].groupby(auth_sc["src_ip"]).size()
        brute_force_ips = set(ip_fails[ip_fails >= 5].index.tolist())

# Suspicious DNS
suspicious_dns_hits = 0
if dns_sc.get("domain") and dns_sc["domain"] in dns.columns:
    suspicious_dns_hits = dns[dns[dns_sc["domain"]].isin(SUSPICIOUS_DOMAINS)].shape[0]
    # Also catch TLD-based suspicious patterns
    suspicious_tld_mask = dns[dns_sc["domain"]].str.contains(r"\.(ru|cn|xyz|pw|tk|ml|ga|cf)$", case=False, na=False, regex=True)
...
# Blocked connections
blocked_count = 0
if fw_sc.get("action") and fw_sc["action"] in fw.columns:
    blocked_count = fw[fw[fw_sc["action"]].str.contains("block|deny|drop|reject", case=False, na=False, regex=True)].shape[0]
...
with st.sidebar:
    ...
    st.markdown(f"<p style='color:{TEXT_SEC};font-size:0.7rem;text-transform:uppercase;letter-spacing:1px'>Auto-Refresh</p>", unsafe_allow_html=True)
    ...
    <div style='font-size:0.73rem;color:{TEXT_SEC};line-height:1.9'>
      ...
      Malware: <b style='color:{TEXT}'>{mal_count}</b> alerts<br>
      <span style='color:{TEXT_SEC}'>Last loaded: </span>
      <b style='color:{TEXT}'>{pd.Timestamp.now().strftime('%H:%M:%S')}</b>
    </div>""", unsafe_allow_html=True)
    st.divider()

    # Threat score gauge in sidebar
...
def apply_time_filter(df, ts_col, window):
    if not ts_col or ts_col not in df.columns or df.empty:
        return df
    t_max = df[ts_col].max()
...
# KPI CARDS ROW
# ─────────────────────────────────────────────────────────────────────────────
total_events = len(fw_f) + len(auth_f) + len(dns_f) + len(mal)
fail_pct = (failed_logins / len(auth) * 100) if len(auth) else 0
...
with tab_timeline:
    ...
    # Malware events pinned on timeline
...
with tab_mal:
    if mal.empty:
        ...
        # Correlate malware hostname with auth/firewall
...
if auto_refresh:
    st.markdown(f"""
    <div style='position:fixed;bottom:1rem;right:1.5rem;background:{CARD_BG};
         border:1px solid {BORDER};border-radius:8px;padding:0.4rem 0.9rem;
